...
class MEDIUMgp(object):
    """ Big initial class to create our Gaussian process """
    def __init__(self, 
                 kernel, means, t, rv, rverr, bis, sig_bis):
        self.kernel = kernel
        ...
        self.y = np.concatenate([rv, bis])
        self.yerr = np.concatenate([rverr, sig_bis])
    ...
    def from_rdb(cls, filename, kernel=SquaredExponential, **kwargs):
        ...
        return cls(kernel, t, rv, rverr, bis, sig_bis)
    ...
    def k11(self, a, x):
        ...
        vc, vr, bc, br = self._scaling_pars(a)
    ...
    def compute_matrix(self, a, yerr=True, nugget=False):
        ...
        print ('Vc:%.2f  Vr:%.2f  Bc:%.2f  Br:%.2f' % tuple(self._scaling_pars(a)))
        if yerr:
        ...
        K1 = np.hstack((K11, K13))
        K3 = np.hstack((K13.T, K33))
        K = np.vstack((K1, K3))
